...
| **Systems Integration and Architecture 1**<br><sub>*Modules 1–6 interactive reviewer*</sub> | Enterprise Information Architecture, IT Governance, Information and Data Modelling, SOA, Microservices, project delivery and technopreneurship, XML/JSON, XML processing and schema validation, Web Services (SOAP/WSDL/UDDI), API architecture (REST/SOAP/GraphQL/gRPC), performance and stress testing, EAI, Middleware, Cloud Computing, and multi-tier deployment — with blueprint, model-layer, scenario, and standards-reference activities. | \`<Vanilla HTML>\`<br>\`<React JSX>\` | [📁 View Folder](Systems%20Integration%20and%20Architecture%201/)<br>[🚀 Open Web App](Systems%20Integration%20and%20Architecture%201/index.html) |
| **Mobile Computing**<br><sub>*Interactive reviewer*</sub> | Mobile concepts and layers, technology evolution through 5G, Modern JavaScript with ES6, the JavaScript language fundamentals beneath it (engines, types and coercion, control flow, functions and `this`), accessible and secure mobile web forms using FormData, progressive web apps and offline caching, and Core Web Vitals performance and mobile security. | \`<Vanilla HTML>\`<br>\`<React JSX>\` | [📁 View Folder](Mobile%20Computing/)<br>[🚀 Open Web App](Mobile%20Computing/index.html) |
...
- **⚡ Zero-Build Option**  
  The vanilla implementations require zero dependencies. Open \`index.html\` and start studying immediately.
...
- Networking 2 cites its primary sources (IETF RFCs, NIST CSRC, IEEE 802 standards, Bluetooth SIG, 3GPP, Wi-Fi Alliance) on each topic. Where the lecture slides are wrong or outdated — a mistyped TLS 1.3 RFC number, MD5/SHA-1/3DES taught as current, a Bluetooth slot given in milliseconds — the reviewer teaches the current standard directly rather than flagging the slide. The full audit of what differs, and why, lives in [`docs/networking2/CONTENT-REVIEW.md`](docs/networking2/CONTENT-REVIEW.md).
- The Mobile Computing reviewer cites MDN, the ECMAScript specification, W3C (service workers, app manifest, WCAG 2.2), 3GPP, and OWASP guidance while keeping its lessons and capstone usable offline.
- Systems Integration and Architecture 1 cites OASIS, W3C (XML, XSD, XPath, SOAP, WSDL), IETF (RFC 8259, RFC 9110, RFC 6749, RFC 7519), NIST, ISO, PMI, and OWASP across its twenty-three module references and its per-topic source lists.
